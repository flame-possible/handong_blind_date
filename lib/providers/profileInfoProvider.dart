...
class ProfileInfoProvider with ChangeNotifier {
  ...
  final List<String> _mbtiProfileList = [
    "선택 안함",
    "ENFP",
    "ENFJ",
    "ENTP",
  ...
  final List<String> _mbtiMatchingList = [
    "ENFP",
    "ENFJ",
    "ENTP",
    "ENTJ",
  ...
  final List<String> _heightProfileList = [
    "~150",
    "150~155",
    "156~160",
    "161~165",
  ...
  final List<String> _heightMatchingList = [
    "~150",
    "150~155",
    "156~160",
    "161~165",
  ...
  final List<String> _facultyProfileList = [
    "선택 안함",
    "GLS",
    "국제어문",
    "경영경제",
  ...
  final List<String> _facultyMatchingList = [
    "GLS",
    "국제어문",
    "경영경제",
    "기계제어",
  ...
  final List<String> _rcProfileList = [
    "선택 안함",
    "카이퍼",
    "열송",
    "장기려",
  ...
  final List<String> _rcMatchingList = [
    "카이퍼",
    "열송",
    "장기려",
    "카마이클",
  ...
  final List<String> _loveLangProfilList = [
    "선택 안함",
    "인정하는 말",
    "함께하는 시간",
    "선물",
...
*/
  ...
  void seeDetailProfile() {
    ...
    if (_seeDetailProfile == false) {
      ...
      _displayedGeneralItemNum =
          _generalPropertyList.length - 1; // 닉네임을 제하고 보여주므로
  ...
  void update() {
    ...
    docref.get().then((DocumentSnapshot documentSnapshot) {
      nicknameSet(
          documentSnapshot.exists ? documentSnapshot.get('Nickname') : '');
      ageSet(documentSnapshot.exists ? documentSnapshot.get('Age') : '');
      genderSet(documentSnapshot.exists ? documentSnapshot.get('Sex') : '');
      longDateSet(documentSnapshot.exists ? documentSnapshot.get('LongDate') : '');
      mbtiSet(documentSnapshot.exists ? documentSnapshot.get('Mbti') : '-');
      heightSet(documentSnapshot.exists ? documentSnapshot.get('Height') : '');
      religionSet(documentSnapshot.exists ? documentSnapshot.get('Religion') : '');
      cigaretteSet(documentSnapshot.exists ? documentSnapshot.get('Cigarette') : '');
      drinkSet(documentSnapshot.exists ? documentSnapshot.get('Drink') : '');
      undergraduateSet(documentSnapshot.exists ? documentSnapshot.get('Undergraduate') : '');
      militarySet(documentSnapshot.exists ? documentSnapshot.get('Military') : '');
      facultySet(documentSnapshot.exists ? documentSnapshot.get('Faculty') : '');
      rcSet(documentSnapshot.exists ? documentSnapshot.get('Rc') : '');
      loveLangSet(documentSnapshot.exists ? documentSnapshot.get('LoveLang') : '-');
    });
  }
